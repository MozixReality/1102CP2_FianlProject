...
public:
	...
	static struct RateAndScores IinJail(const pair<short, short> &playerAt, const pair<short, short> &movedPlayerAt, const pair<int, int> &scores, const unsigned short depth);
...
void Bot::_computePlayerHalfDistance(){
	vector<vector<bool>> isVisited(gameMap.height());
	for(auto &row: isVisited){
		row.resize(gameMap.width());
	}
	isVisited[gameMap.oppoLocation.first][gameMap.oppoLocation.second] = true;
	...
	while(!q.empty()){
		for(short dir=0 ; dir < 4 ; ++dir){
			...
			if(movedPlayerAt.first < 0 || movedPlayerAt.first >= gameMap.height() || movedPlayerAt.second < 0 || movedPlayerAt.second >= gameMap.width() ||
					...
					isVisited[movedPlayerAt.first][movedPlayerAt.second]){
				continue;
			}
			isVisited[movedPlayerAt.first][movedPlayerAt.second] = true;
